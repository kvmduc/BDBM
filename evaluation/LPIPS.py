...
def calc_LPIPS(data_dir, gt_dir, num_samples=1):
    dir_list = [name for name in os.listdir(gt_dir) if name.endswith('.png') and not name.endswith('_normal.png')]
    dir_list.sort()
    
    gen_dir_list = [name for name in os.listdir(data_dir) if name.endswith('.png')]

    total = min(len(gen_dir_list), len(dir_list))
    
    total_lpips_distance = 0
    for i in tqdm(range(total), total=total, smoothing=0.01):
        try:
            gt_name = os.path.join(gt_dir, f'{str(gen_dir_list[i][:-4])}.png')
            
            gt_img = lpips.im2tensor(lpips.load_image(gt_name)).to(torch.device('cuda:0'))
            
            # gt_img = lpips.im2tensor(cv2.resize(cv2.imread(gt_name), (256,256))).to(torch.device('cuda:0'))
            for j in range(num_samples):
                if num_samples == 1:
                    img_name = os.path.join(os.path.join(data_dir, f'{str(gen_dir_list[i][:-4])}.png'))
                else:
