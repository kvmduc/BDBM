...
class BaseRunner(ABC):
    ...
    def load_model_from_checkpoint(self):
        ...
        if self.config.model.__contains__('model_load_path') and self.config.model.model_load_path is not None:
            ...
            # load model
            model_state = model_states['model']
    ...
    def validation_step(self, val_batch, epoch, step):
        ...
        try:
            wandb.log({"val_loss_step": loss_val, "val_logloss_step": np.log(loss_val)}, step=self.global_step)
        except:
            pass
        ...
        try:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for train_batch in pbar:
                    ...
                    if len(self.optimizer) > 1:
                        ...
                        if not self.config.training.use_DDP or \
                            ...
                            losses_to_log.append(loss.detach().item())
                            
                            if self.global_step % 200 == 0:
                                try:
                                    wandb.log({"train_loss": np.mean(losses_to_log), "train_logloss": np.log(np.mean(losses_to_log))}, step=self.global_step)
                                except:
                                    pass
                                losses_to_log = []
                ...
                if (epoch + 1) % self.config.training.validation_interval == 0 or (
                    ...
                    if not self.config.training.use_DDP or \
                            (self.config.training.use_DDP and self.config.training.local_rank) == 0:
                                # LOGGED to WANDB : validation loss
                        with torch.no_grad():
                            print("validating epoch...")
                            average_loss = self.validation_epoch(val_loader, epoch)
                            try:
                                wandb.log({"val_loss_epoch": average_loss})
                            except:
                                pass
                            torch.cuda.empty_cache()
    ...
    def test(self):
        ...
        if self.config.args.sample_to_eval:
            ...
            # sample_path = os.path.join(self.config.result.sample_to_eval_path, 'val')
            if self.config.training.use_DDP:
