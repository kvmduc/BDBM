...
class LatentBrownianBridgeModel(BrownianBridgeModel):
    ...
    def forward(self, x, x_cond, context=None):
        with torch.no_grad():
            x_latent = self.encode(x, cond=False)               # encoded x0
            x_cond_latent = self.encode(x_cond, cond=True)      # encoded y
        '''
        self.condition_key == 'None'                -> context = None
        self.condition_key == 'first_stage'         -> context = VQGAN(y)
        self.condition_key == 'SpatialRescaler'     -> context = SpatialRescaler(y)
        '''    
